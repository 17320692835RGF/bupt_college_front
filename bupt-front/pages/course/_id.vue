                          ...
                          <li class="lh-menu-stair" v-for="chapter in chapterVoList" :key="chapter.id">
                            ...
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <div v-if="video.isFree == 0 && isbuy == false">
                                    <em class="lh-menu-i-2 icon16 mr5">&nbsp;</em>{{ video.title }}
                                </div>
                                <div v-if="video.isFree == 1">
                                ...
                                <div v-if="video.isFree == 0 && isbuy == true">
                                  <a :href="'/player/'+video.videoSourceId" target="_blank">
                                    <em class="lh-menu-i-2 icon16 mr5">&nbsp;</em>{{ video.title }}
                                  </a>
                                </div>
            ...
            <li class="unBr">
              ...
              <div class="of">
                <section class="n-reply-wrap">
                  ...
                  <p class="of mt5 tar pl10 pr10"><span class="fl "><tt class="c-red comment Contentmeg"
          ...
          <section class="question-list lh-bj-list pr">
            <ul class="pr10">
              <li v-for="(comment,index) in data.items" v-bind:key="index">
                ...
                <div class="of"><span class="fl"><font class="fsize12 c-blue">{{ comment.nickname }}</font><font class="fsize12 c-999 ml5">评论：</font></span>
                </div>
                <div class="noter-txt mt5">
                  <p>{{ comment.content }}</p>
                </div>
                <div class="of mt5"><span class="fr"><font class="fsize12 c-999ml5">{{ comment.gmtCreate }}</font></span>
                </div>
              </li>
            </ul>
          </section>
...
export default {
  asyncData({params, error}) {
    // return course.getCourseById(params.id).then(response => {
    //   console.log(response.data);
    //   return {
    //     course: response.data.course,
    //     chapterVoList: response.data.chapterVoList,
  ...
  methods: {
    ...
    createOrders(){
      order.createOrder(this.$route.params.id).then(response => {
        if (response.success) {
    ...
    initComment() {
      comment.getCommentList(this.$route.params.id, this.page, this.limit).then(response => {
        console.log(response.data);
    ...
    gotoPage(page) {
      comment.getCommentList(this.$route.params.id, page, this.limit).then(response => {
        this.data = response.data
      })
    }
  }
